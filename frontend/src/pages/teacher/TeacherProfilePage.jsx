...
import { User, Lock, Phone, Mail, Building2, Award } from 'lucide-react';
...
export const TeacherProfilePage = () => {
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      <div>
        <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight">Faculty Profile & Settings</h1>
        <p className="text-xs text-slate-500 mt-1">Manage institutional credentials and portal access security</p>
      </div>
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-sm flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-6">
        <img
          src={user?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.full_name || 'Teacher')}&background=4f46e5&color=fff`}
          alt={user?.full_name}
        ...
        <div className="flex-1 text-center sm:text-left space-y-1">
          ...
          <p className="text-xs font-mono font-bold text-indigo-600">Employee ID: {profile?.employee_id}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-sm space-y-4">
          ...
          <form onSubmit={handleUpdateProfile} className="space-y-4 text-xs">
            <div>
              <label className="block font-semibold text-slate-700 mb-1">Official Name (Read-Only)</label>
              <input
                type="text"
                disabled
            ...
            <div>
              <label className="block font-semibold text-slate-700 mb-1">Institutional Email (Read-Only)</label>
              <input
                type="email"
                disabled
                value={user?.email || ''}
